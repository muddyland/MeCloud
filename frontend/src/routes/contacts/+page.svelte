        ...
        {#if newBookInput}
          <div class="flex items-center gap-1 px-2 py-1">
            <input bind:value={newBookName}
              ...
              autofocus />
            <button on:click={confirmNewBook} disabled={creatingBook} title="Create"
              class="text-xs text-blue-600 dark:text-blue-400 disabled:opacity-50
                     w-4 flex items-center justify-center">
              {#if creatingBook}<Spinner size="xs" label="Creating" />{:else}✓{/if}
            </button>
          ...
          <div class="group relative">
            ...
            <button on:click={() => confirmDeleteBook(book)} disabled={deletingBookId === book.id}
              ...
              class="absolute right-1 inset-y-0 flex items-center px-1 transition-colors
                     ...
                     {deletingBookId === book.id ? '' : 'hidden group-hover:flex'}">
              {#if deletingBookId === book.id}
                <Spinner size="xs" label="Deleting address book" accent="border-t-red-500" />
              {:else}
          ...
          {#each Array(8) as _}
            <div class="flex items-center gap-3 px-4 py-3 border-b border-gray-100 dark:border-gray-800">
              ...
              <div class="flex-1 min-w-0 space-y-1.5">
                ...
                <div class="h-2.5 rounded bg-gray-100 dark:bg-gray-700/60 animate-pulse w-3/4"></div>
              </div>
            </div>
          {/each}
        {:else if filtered.length === 0}
          <div class="flex flex-col items-center justify-center h-40 gap-2 text-gray-400 dark:text-gray-500">
            <svg class="w-9 h-9 opacity-40" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                 stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
              <path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/>
            </svg>
            <span class="text-sm">{$contactSearch ? 'No matching contacts' : 'No contacts yet'}</span>
          </div>
        {:else}
